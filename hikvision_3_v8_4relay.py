...
class DetectionThread(QThread):
    ...
    def process_frame(self, frame):
        try:
            ...
            # NEW LOGIC: Any missing or mismatch = problem
            for i in range(3):
                ...
                # NEW: Only OK if BOTH are present
                # Any other condition (both absent, only one present) = NOT OK
                if oc_present and bh_present:
                    ...
                    if not oc_present and not bh_present:
                        pair_status = 'both_absent'
                    elif oc_present and not bh_present:
                        pair_status = 'bh_missing'
                    else:  # bh_present and not oc_present
                        pair_status = 'oc_missing'
...
class RelayControlThread(QThread):
    ...
    def run(self):
        self.running = True
        logger.info("Relay control thread started (4-relay mode)")
    ...
    def execute_command(self, relay_states):
        """
        relay_states is a dict:
        {
            'pair1_ok': bool,
            'pair2_ok': bool,
            'pair3_ok': bool,
        ...
        Relay 1 = Pair 1 (ON when problem)
        Relay 2 = Pair 2 (ON when problem)
        Relay 3 = Pair 3 (ON when problem)
        Relay 4 = All OK indicator (ON when all_ok)
        """
        try:
            state_key = (
                relay_states['pair1_ok'],
                relay_states['pair2_ok'],
                relay_states['pair3_ok'],
                relay_states['all_ok']
            )
            ...
            # Relay 1: ON when Pair 1 has problem
            self.relay.set_state(1, not relay_states['pair1_ok'])
            
            # Relay 2: ON when Pair 2 has problem
            self.relay.set_state(2, not relay_states['pair2_ok'])
            
            # Relay 3: ON when Pair 3 has problem
            self.relay.set_state(3, not relay_states['pair3_ok'])
            
            # Relay 4: ON when everything is OK
            self.relay.set_state(4, relay_states['all_ok'])
            
            status_parts = []
            if not relay_states['pair1_ok']:
                status_parts.append("P1")
            if not relay_states['pair2_ok']:
                status_parts.append("P2")
            if not relay_states['pair3_ok']:
                status_parts.append("P3")
            
            if relay_states['all_ok']:
                logger.info("Relays: All OK (R1=OFF, R2=OFF, R3=OFF, R4=ON)")
                self.status_signal.emit("Relay: All Pairs OK")
            else:
                problem_str = ", ".join(status_parts)
                logger.warning(f"Relays: Problems detected [{problem_str}] (R4=OFF)")
                self.status_signal.emit(f"Relay: Problems in {problem_str}")
    ...
    def set_state(self, relay_states):
        try:
            self.command_queue.put(relay_states, block=False)
        except queue.Full:
            pass
...
class TrainingPage(QWidget):
    ...
    def init_ui(self):
        ...
        instructions = QLabel(
            ...
            "3. Each pair (OC1<->BH1, OC2<->BH2, OC3<->BH3) controls its own relay\n"
            "4. NEW LOGIC: Problem = BOTH absent OR mismatch (one present, one absent)\n"
            "5. Relay turns ON when there's a problem in that pair\n"
            "6. Relay 4 turns ON only when ALL pairs are OK\n"
    ...
    def save_boundaries(self):
        ...
        try:
            ...
            QMessageBox.information(
                ...
                f"4-Relay Logic:\n"
                f"Relay 1: Pair 1 problem indicator\n"
                f"Relay 2: Pair 2 problem indicator\n"
                f"Relay 3: Pair 3 problem indicator\n"
                f"Relay 4: All OK indicator"
            )
        except Exception as e:
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle("Industrial Detection System v8.0 - 4-Relay Problem Detection")
        ...
        self.statusBar().showMessage("4-Relay System Ready - Problem = Absent OR Mismatch")
        
        logger.info("="*60)
        logger.info("APPLICATION STARTED - v8.0 4-RELAY SYSTEM")
        logger.info("="*60)
    ...
    def create_menu_bar(self):
        ...
        test_relay_action = QAction('Test 4 Relays', self)
    ...
    def test_relay(self):
        try:
            ...
            reply = QMessageBox.question(
                self, 'Test 4 Relays',
                'This will test all 4 relays sequentially.\nContinue?',
                QMessageBox.Yes | QMessageBox.No
            )
    ...
    def show_about(self):
        QMessageBox.about(
            self, "About",
            "Industrial Detection System v8.0\n\n"
            "4-Relay Problem Detection System\n"
            "Supports USB and IP Cameras (RTSP)\n\n"
            "NEW Detection Logic:\n"
            "- Problem = BOTH absent OR mismatch\n"
            "- OK = BOTH present only\n\n"
            "Relay Configuration:\n"
            "- Relay 1: Pair 1 problem indicator (ON = Problem)\n"
            "- Relay 2: Pair 2 problem indicator (ON = Problem)\n"
            "- Relay 3: Pair 3 problem indicator (ON = Problem)\n"
            "- Relay 4: All OK indicator (ON = All OK)\n\n"
            "Model Requirements:\n"
            "- Class 0: oil_can\n"
            "- Class 1: bunk_hole\n\n"
            "Features:\n"
            ...
            "- Problem detection (absent + mismatch)\n"
            "- Auto error recovery\n"
...
def main():
    ...
    logger.info("="*60)
    logger.info("INDUSTRIAL DETECTION SYSTEM v8.0 STARTED")
    logger.info("4-RELAY PROBLEM DETECTION SYSTEM")
    logger.info("="*60)
    logger.info("Logic: Problem = Both Absent OR Mismatch")
    logger.info("Relay 1-3: Individual pair problems (ON = Problem)")
    logger.info("Relay 4: All OK indicator (ON = All OK)")
    logger.info("="*60)
